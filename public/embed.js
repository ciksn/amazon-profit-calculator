...
function optionalYesNoLabel(value){return value==null?'':Number(value)?'是':'否'}
function sellingPointAnalysisBusy(){return Boolean(state.analyzingSiteCode||state.similarAnalyzingSiteCode)}
...
async function analyzeCompetitors(code,kind='standard'){
  const similar=kind==='similar',stateKey=similar?'similarAnalyzingSiteCode':'analyzingSiteCode',endpoint=similar?'similar-competitors/analyze':'competitors/analyze';
  if(sellingPointAnalysisBusy())return;state[stateKey]=code;renderCompetitors();renderSimilarCompetitors();saving(true);
  try{let result;for(let attempt=0;attempt<3;attempt+=1){try{result=await api(`/api/projects/${state.project.id}/${endpoint}`,{method:'POST',body:JSON.stringify({country_code:code})});break}catch(error){const transient=!error.status||error.status===408||error.status===425||error.status===429||error.status>=500;if(!transient||attempt===2)throw error;toast(`网络波动，正在重试 ${attempt+1}/2…`);await new Promise((resolve)=>setTimeout(resolve,1200*(attempt+1)))}}await loadCompetitors();saving(false);const failed=failedAnalysisRows(code,null,kind);const summary=result.attempted===0?`前五竞品已有卖点，无需重复分析`:`本次分析 ${result.analyzed}/${result.attempted} 条${result.skipped?`，跳过已有结果 ${result.skipped} 条`:''}`;toast(failed.length?`${summary}，请补充 ${failed.length} 条竞品五点`:`${summary}，分析完成`);if(failed.length)openManualAnalysis(code,failed,kind)}
  catch(error){saving(false,true);toast(error.message)}finally{state[stateKey]='';renderCompetitors();renderSimilarCompetitors()}
}
...
function failedAnalysisRows(code,ids=null,kind='standard'){
  const allowed=ids?new Set(ids.map(Number)):null;
  const rows=kind==='similar'?similarRowsFor(code):competitorRowsFor(code);
  return rows.slice(0,5).filter((item)=>(!allowed||allowed.has(Number(item.id)))&&item.analysis_status==='insufficient');
}
...
function openManualAnalysis(code,rows,kind='standard'){
  rows=rows||failedAnalysisRows(code,null,kind);
  if(!rows.length)return;state.manualAnalysisCountryCode=code;state.manualAnalysisIds=rows.map((item)=>Number(item.id));state.manualAnalysisKind=kind;
...
function closeManualAnalysis(){$('#manualAnalysisModal').hidden=true;state.manualAnalysisCountryCode='';state.manualAnalysisIds=[];state.manualAnalysisKind='standard';$('#manualAnalysisError').textContent=''}
async function submitManualAnalysis(event){
  event.preventDefault();const code=state.manualAnalysisCountryCode,kind=state.manualAnalysisKind;if(!code)return;
  ...
  try{const endpoint=kind==='similar'?'similar-competitors/analyze':'competitors/analyze';await api(`/api/projects/${state.project.id}/${endpoint}`,{method:'POST',body:JSON.stringify({country_code:code,manual_rows:manualRows})});await loadCompetitors();const failed=failedAnalysisRows(code,state.manualAnalysisIds,kind);saving(false);if(!failed.length){closeManualAnalysis();toast('手动补充的竞品已全部分析成功');return}state.manualAnalysisIds=failed.map((item)=>Number(item.id));$('#manualAnalysisRows').innerHTML=failed.map(manualAnalysisRow).join('');$('#manualAnalysisError').textContent=`仍有 ${failed.length} 条竞品未生成合规卖点，请检查五点内容后重试。`}
