...
async function analyzeCompetitors(code,kind='standard'){
  const similar=kind==='similar',stateKey=similar?'similarAnalyzingSiteCode':'analyzingSiteCode',render=similar?renderSimilarCompetitors:renderCompetitors,endpoint=similar?'similar-competitors/analyze':'competitors/analyze';
  if(state[stateKey])return;state[stateKey]=code;render();saving(true);
  ...
  catch(error){saving(false,true);toast(error.message)}finally{state[stateKey]='';render()}
}
