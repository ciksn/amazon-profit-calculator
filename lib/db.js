...
function initialize() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS projects (
      ...
      owner_name TEXT NOT NULL DEFAULT '', parent_asin TEXT NOT NULL DEFAULT '', child_asin TEXT NOT NULL DEFAULT '',
      sales_amount_cny REAL NOT NULL DEFAULT 0, six_day_capacity REAL NOT NULL DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS project_competitors (
      ...
      name TEXT NOT NULL DEFAULT '', sale_price REAL NOT NULL DEFAULT 0,
      cost_cny REAL NOT NULL DEFAULT 0, length REAL NOT NULL DEFAULT 0,
      width REAL NOT NULL DEFAULT 0, height REAL NOT NULL DEFAULT 0,
      dimension_unit TEXT NOT NULL DEFAULT 'cm', weight REAL NOT NULL DEFAULT 0,
      weight_unit TEXT NOT NULL DEFAULT 'kg', category_text TEXT NOT NULL DEFAULT '',
      uses_project_defaults INTEGER NOT NULL DEFAULT 1,
      ...
      monthly_sales REAL NOT NULL DEFAULT 0, monthly_revenue_local REAL NOT NULL DEFAULT 0,
      monthly_revenue_usd REAL NOT NULL DEFAULT 0, rating REAL, source_format TEXT NOT NULL DEFAULT '',
      source_row INTEGER NOT NULL DEFAULT 0,
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_project_competitors_project
      ON project_competitors(project_id, country_code, id);
  ...
  const projectColumns = new Set(db.prepare('PRAGMA table_info(projects)').all().map((item) => item.name));
  if (!projectColumns.has('image_data')) db.exec("ALTER TABLE projects ADD COLUMN image_data TEXT NOT NULL DEFAULT ''");
  if (!projectColumns.has('owner_name')) db.exec("ALTER TABLE projects ADD COLUMN owner_name TEXT NOT NULL DEFAULT ''");
  if (!projectColumns.has('parent_asin')) db.exec("ALTER TABLE projects ADD COLUMN parent_asin TEXT NOT NULL DEFAULT ''");
  if (!projectColumns.has('child_asin')) db.exec("ALTER TABLE projects ADD COLUMN child_asin TEXT NOT NULL DEFAULT ''");
  if (!projectColumns.has('sales_amount_cny')) db.exec('ALTER TABLE projects ADD COLUMN sales_amount_cny REAL NOT NULL DEFAULT 0');
  if (!projectColumns.has('six_day_capacity')) db.exec('ALTER TABLE projects ADD COLUMN six_day_capacity REAL NOT NULL DEFAULT 0');
  const competitorColumns = new Set(db.prepare('PRAGMA table_info(project_competitors)').all().map((item) => item.name));
  if (!competitorColumns.has('category_text')) db.exec("ALTER TABLE project_competitors ADD COLUMN category_text TEXT NOT NULL DEFAULT ''");
  if (!competitorColumns.has('uses_project_defaults')) db.exec('ALTER TABLE project_competitors ADD COLUMN uses_project_defaults INTEGER NOT NULL DEFAULT 1');
  const competitorImportColumns = {
    asin:"TEXT NOT NULL DEFAULT ''",image_url:"TEXT NOT NULL DEFAULT ''",product_url:"TEXT NOT NULL DEFAULT ''",
    is_fba:'INTEGER',has_aplus:'INTEGER',has_video:'INTEGER',listing_date:"TEXT NOT NULL DEFAULT ''",
    monthly_sales:'REAL NOT NULL DEFAULT 0',monthly_revenue_local:'REAL NOT NULL DEFAULT 0',
    monthly_revenue_usd:'REAL NOT NULL DEFAULT 0',rating:'REAL',source_format:"TEXT NOT NULL DEFAULT ''",
  ...
  for (const [column,type] of Object.entries(competitorImportColumns)) {
    if (!competitorColumns.has(column)) db.exec(`ALTER TABLE project_competitors ADD COLUMN ${column} ${type}`);
  }
