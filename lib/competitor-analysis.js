...
function validateGeminiProducts(payload,expectedIds) {
  ...
  for(const item of payload.products){
    ...
    rows.push({id,sellingPoints,differentiation,status:sellingPoints.length>=3?'complete':'insufficient'});
  }
...
async function callGemini(items,{apiKey=getGeminiApiKey(),model=process.env.GEMINI_MODEL||DEFAULT_MODEL,client,
  ...
  try {
    ...
    {timeout:safeTimeout,maxRetries:Math.min(3,Math.max(0,Number(process.env.GEMINI_MAX_RETRIES??2))),fetchOptions:dispatcher?{dispatcher}:undefined});
...
async function analyzeCompetitorBatch(rows,options={}) {
  ...
  const prepared=await mapLimit(rows,2,async(row)=>{
    const title=String(row.name||'').trim();const productUrl=canonicalProductUrl(row);let bullets=[];let scrapeError='';
    if(title&&productUrl){try{bullets=await fetchAmazonBulletsWithRetry(productUrl,row.country_code,options)}catch(error){scrapeError=error.message}}
  ...
  return {model,rows:prepared.map((item)=>{const result=byId.get(Number(item.row.id));return {id:Number(item.row.id),featureBullets:item.bullets,
    sellingPoints:result?.sellingPoints||[],differentiation:result?.differentiation||[],status:result?.status||'insufficient',warning:item.scrapeError||(!item.title?'缺少标题':!item.productUrl?'缺少有效 Amazon 链接':'')}})};
}
