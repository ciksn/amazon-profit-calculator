...
function isInvalidPreviousResponseError(error) {
  const status=Number(error?.status);
  if (status!==400&&status!==404) return false;
  const detail=error?.error&&typeof error.error==='object'?error.error:{};
  const param=String(error?.param||detail.param||'').toLowerCase();
  if (param==='previous_response_id') return true;
  const code=String(error?.code||detail.code||'').toLowerCase();
  if (code==='previous_response_not_found'||code==='invalid_previous_response_id') return true;
  const diagnostic=`${code} ${String(error?.message||'')} ${String(detail.message||'')}`;
  return /previous_response(?:_id)?/i.test(diagnostic)
    &&/(?:not found|does not exist|invalid|expired)/i.test(diagnostic);
}
...
function createOpenAiProvider({client:providedClient,model:providedModel}={}) {
  ...
  async function* streamTurn({state={},system='',input='',turnId:requestedTurnId,signal}={}) {
    const sdk=getClient();
    ...
    try {
      if (controller.signal.aborted) throw providerError('OPENAI_INTERRUPTED','OpenAI turn was interrupted');
      const conversation=await startOrResumeConversation(state);
      if (controller.signal.aborted) throw providerError('OPENAI_INTERRUPTED','OpenAI turn was interrupted');
      let previousResponseId=conversation.openai_state_id;
      let retriedInvalidState=false;
      while (true) {
        const request={
          model:getModel(),
          input:[
        ...
        try {
          ...
          if (!retriedInvalidState&&previousResponseId&&isInvalidPreviousResponseError(error)) {
            retriedInvalidState=true;
