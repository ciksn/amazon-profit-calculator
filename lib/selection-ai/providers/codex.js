...
function isMissingThreadError(error) {
  ...
  return /no rollout found for thread id\b/i.test(message)
    || /\bthread(?: id)?\b.*\b(?:not found|does not exist)\b/i.test(message);
}
...
function createCodexProvider({
  ...
  function route(target,message) {
    ...
    if (message&&message.id!==undefined) {
      ...
      clearTimeout(request.timer);
      if (message.error) {
        const failure=providerError(request.errorCode,request.errorMessage);
        failure.missingThread=request.method==='thread/resume'
          &&isMissingThreadError(message.error);
  ...
  function request(method,params,errorCode,errorMessage,requestTimeout=timeoutMs) {
    ...
    return new Promise((resolve,reject)=>{
      ...
      pending.set(id,{resolve,reject,timer,errorCode,errorMessage,method,transport:target});
  ...
  async function startOrResumeConversation(state={},options={}) {
    ...
    if (savedThreadId) {
      try {
        result=await request(
          'thread/resume',{threadId:savedThreadId,developerInstructions},
          'CODEX_START_FAILED','Codex conversation could not be started'
        );
        resumedExistingThread=true;
      } catch (error) {
        if (!error?.missingThread) throw error;
        result=await request(
    ...
    const threadId=result?.thread?.id||(resumedExistingThread?savedThreadId:null);
