...
function createSelectionAiService({db,repository,providers,loadPayload}) {
  ...
  async function health(projectId) {
    ...
    const [codex,openai]=await Promise.all([
      probeProviderHealth(providers.codex),
      probeProviderHealth(providers.openai)
    ]);
    return {active_provider:provider,providers:{codex,openai}};
  }
...
function selectionAiProviderConfig(env=process.env) {
  ...
  return {
    codex:{
      command:String(env.CODEX_COMMAND||'').trim()||'codex',
