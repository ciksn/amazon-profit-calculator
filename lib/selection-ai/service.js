...
function appendDeterministicSummary(summary,messages) {
  const additions=[];
  for (const message of messages) {
    const role=message?.role==='assistant'?'assistant':'user';
    const content=String(message?.content??'').replace(/\s+/g,' ').trim().slice(0,1000);
    additions.push(`${role}: ${content}`);
  }
  const combined=[String(summary??'').trimEnd(),additions.join('\n')]
    .filter(Boolean)
    .join('\n');
  if (combined.length<=MAX_SUMMARY_LENGTH) return combined;
  const start=combined.length-MAX_SUMMARY_LENGTH;
  if (start===0||combined[start-1]==='\n') return combined.slice(start);
  const nextLine=combined.indexOf('\n',start);
  return nextLine===-1?combined.slice(-MAX_SUMMARY_LENGTH):combined.slice(nextLine+1);
}
...
function createSelectionAiService({db,repository,providers,loadPayload}) {
  ...
  async function* streamTurn({projectId,chapter='overview',message='',signal}={}) {
    ...
    try {
      ...
      const messages=await repository.listRecentMessages(projectId,20);
      throwIfAborted(abortController.signal);
      let summary=initialState.conversation.summary||'';
      let summaryMessageId=Math.max(
        0,Math.floor(Number(initialState.conversation.summary_message_id)||0)
      );
      let rebuildLegacySummary=summaryMessageId===0&&Boolean(summary);
      const beforeId=messages.length===20?Number(messages[0].id):0;
      while (beforeId>summaryMessageId) {
        throwIfAborted(abortController.signal);
        const page=await repository.listMessagesForSummary(projectId,{
          afterId:summaryMessageId,beforeId,limit:SUMMARY_PAGE_LENGTH
        });
        throwIfAborted(abortController.signal);
        if (!page.length) break;
        if (rebuildLegacySummary) {
          summary='';
          rebuildLegacySummary=false;
        }
        summary=appendDeterministicSummary(summary,page);
        summaryMessageId=Number(page.at(-1).id);
        await repository.setSummary(projectId,summary,summaryMessageId);
        throwIfAborted(abortController.signal);
      }
  ...
  async function applyProposal({projectId,proposalId,changeIndexes}={}) {
    ...
    return db.transaction(async(client)=>{
      const proposal=await selectOne(client,
        'SELECT * FROM selection_ai_proposals WHERE id=$1 AND owner_user_id=$2 FOR UPDATE',[proposalId,currentOwnerId()]
      );
      ...
      const document=await selectOne(client,
        'SELECT * FROM selection_documents WHERE project_id=$1 AND owner_user_id=$2 FOR UPDATE',[projectId,currentOwnerId()]
      );
      ...
      for (const [countryCode,fieldMap] of siteValues) {
        const currentSite=await selectOne(client,
          'SELECT * FROM selection_site_assessments WHERE project_id=$1 AND country_code=$2 AND owner_user_id=$3 FOR UPDATE',
          [projectId,countryCode,currentOwnerId()]
        );
      ...
      const documentUpdate=await client.query(
        `UPDATE selection_documents SET ${assignments.join(',')}
         WHERE project_id=$${values.length+2} AND version=$${values.length+3} AND owner_user_id=$${values.length+4} RETURNING *`,
        [...values,updatedAt,projectId,proposal.base_document_version,currentOwnerId()]
      );
      ...
      for (const [countryCode,fieldMap] of siteValues) {
        ...
        const siteParams=[
          projectId,currentOwnerId(),countryCode,
          ...siteFields.map((field)=>fieldMap.get(field).after),
          updatedAt,
          ...siteFields.map((field)=>fieldMap.get(field).before)
        ];
        const columns=['project_id','owner_user_id','country_code',...siteFields,'updated_at'];
        const placeholders=['$1','$2','$3',
          ...siteFields.map((_,index)=>`$${index+4}`),`$${siteFields.length+4}`];
        const updatedAtIndex=siteFields.length+4;
        const beforeStartIndex=siteFields.length+5;
        const updates=[...siteFields.map((field,index)=>`${field}=$${index+4}`),`updated_at=$${updatedAtIndex}`];
        ...
        const siteUpdate=await client.query(
          `INSERT INTO selection_site_assessments (${columns.join(',')}) VALUES (${placeholders.join(',')})
           ...
           WHERE selection_site_assessments.owner_user_id=EXCLUDED.owner_user_id AND ${predicates.join(' AND ')} RETURNING *`,
          siteParams
        );
