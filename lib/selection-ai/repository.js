...
function createSelectionAiRepository(db) {
  async function projectExists(projectId,client=db) {
    return Boolean(await one(client,'SELECT id FROM projects WHERE id=$1',[projectId]));
  }
  ...
  async function listMessagesForSummary(projectId,{afterId=0,beforeId,limit=200}={}) {
    ...
    return db.many(
      ...
      [projectId,after,before,historyLimit(limit)]
    );
  }
  ...
  async function setSummary(projectId,summary,summaryMessageId) {
    ...
    if (summaryMessageId===undefined) {
      return one(db,
        'UPDATE selection_ai_conversations SET summary=$1,updated_at=$2 WHERE project_id=$3 RETURNING *',
        [String(summary??''),now(),projectId]
      );
    }
    ...
    return one(db,
      'UPDATE selection_ai_conversations SET summary=$1,summary_message_id=$2,updated_at=$3 WHERE project_id=$4 RETURNING *',
      [String(summary??''),cursor,now(),projectId]
    );
  }
