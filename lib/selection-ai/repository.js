...
function createSelectionAiRepository(db) {
  ...
  async function createMessage(input={}) {
    ...
    return one(db,
      ...
      [input.projectId,String(input.role??''),String(input.provider??''),String(input.content??''),String(input.status??'pending'),String(input.errorCode??''),String(input.errorMessage??''),createdAt]
    );
  }
  ...
  async function updateMessage(id,patch={}) {
    ...
    if (!fields.length) return one(db,'SELECT * FROM selection_ai_messages WHERE id=$1',[id]);
    const values=fields.map(([key])=>String(patch[key]??''));
    values.push(now(),id);
    const assignments=fields.map(([,column],index)=>`${column}=$${index+1}`);
    assignments.push(`updated_at=$${values.length-1}`);
    return one(db,`UPDATE selection_ai_messages SET ${assignments.join(',')} WHERE id=$${values.length} RETURNING *`,values);
  }

  async function createProposal(input={}) {
    return normalizeProposal(await one(db,
