...
const STRUCTURED_OUTPUT_ERROR = '结构化输出无效';
...
function createStructuredAnswerStream() {
  ...
  function rejectStructuredOutput() {
    failure = true;
    throw new Error(STRUCTURED_OUTPUT_ERROR);
  }
  ...
  function closeString() {
    if (stringKind === 'rootKey') {
      ...
      if (rootKey === 'answer') {
        if (answerSeen) rejectStructuredOutput();
        answerSeen = true;
      }
      rootState = 'colon';
    } else if (stringKind === 'answer') {
      if (pendingHighSurrogate) rejectStructuredOutput();
      finishRootValue();
  ...
  function readStringCharacter(character, output) {
      ...
      if (unicodeDigits.length === 4) {
        if (/^[0-9a-fA-F]{4}$/.test(unicodeDigits) && stringKind === 'answer') {
          ...
          if (codeUnit >= 0xD800 && codeUnit <= 0xDBFF) {
            if (pendingHighSurrogate) rejectStructuredOutput();
            pendingHighSurrogate = String.fromCharCode(codeUnit);
          } else if (codeUnit >= 0xDC00 && codeUnit <= 0xDFFF) {
            if (!pendingHighSurrogate) rejectStructuredOutput();
            output.push(pendingHighSurrogate, String.fromCharCode(codeUnit));
    ...
    if (escapeState === 'escape') {
      ...
      if (stringKind === 'answer' && Object.hasOwn(escapes, character)) {
        if (pendingHighSurrogate) rejectStructuredOutput();
        output.push(escapes[character]);
      }
      escapeState = null;
      return;
    }
  ...
  function push(delta) {
    if (failure) throw new Error(STRUCTURED_OUTPUT_ERROR);
