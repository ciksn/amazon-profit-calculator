...
function calculateProfit({ project, country, listing, fbaRules = [], sizeTiers = [], freightRule = null }) {
  ...
  if (fbaRule) {
    ...
    calculatedFbaFee = fbaBaseFee + fbaSurchargeFee;
  }
  const hasFbaOverride = listing.fba_fee_override !== null && listing.fba_fee_override !== undefined && listing.fba_fee_override !== '';
  const fbaFee = hasFbaOverride ? Math.max(0,Number(listing.fba_fee_override) || 0) : calculatedFbaFee;
  ...
  if (!fbaRule && !hasFbaOverride) warnings.push('暂无可用 FBA 规则');
  ...
  return {
    ...
    fba_calculated_fee: round(calculatedFbaFee),
    fba_fee_overridden: hasFbaOverride,
...
module.exports = { calculateProfit, findSalePriceForProfitRate, normalizeDimensions, normalizeWeight, selectFbaRule, selectSizeTier, selectStrictFbaRule };
