...
function extractTopReviews(html,{limit=MAX_TOP_REVIEWS}={}) {
  ...
  $(selector).each((_,element)=>{
    ...
    const body=cleanText(root.find('[data-hook="review-body"]').first().text(),4000);
    if(!body)return;
    ...
    reviews.push({
      ...
      title:cleanText(root.find('[data-hook="review-title"]').first().text().replace(ratingText,''),500),
      body,
