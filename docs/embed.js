...
function profitInfoIcon(result){
  ...
  const explanation=[
    '???????',
    `?????${money(result.sale_price)}`,
    `? VAT?${money(result.vat_amount)}`,
    `??????${money(result.net_revenue)}`,
    `? ${result.tax_label||'??'}?${money(result.tax_fee)}`,
    `????${money(result.referral_fee)}?${number(result.referral_rate,2)}%?`,
    `? FBA?${money(result.fba_fee)}`,
    `????${money(result.freight_fee)}`,
    `??????${money(result.product_cost)}`,
    `?????${money(result.profit)}`,
    `????${money(result.profit)} ? ${money(result.sale_price)} ? 100 = ${number(result.profit_rate,1)}%`
  ].join('\n');
  return `<span class="profit-info" tabindex="0" title="${escapeHtml(explanation)}" aria-label="?????????">i</span>`;
}
...
async function importSnapshot(payload,encoded){
  if(payload?.v!==1||!payload.product||!Array.isArray(payload.listings))throw new Error('?????????');
  ...
  if(!project){project=await api('/api/projects',{method:'POST',body:JSON.stringify({name:payload.product.name||'?????'})});localStorage.setItem(mappingKey,String(project.id))}
...
async function initialize(){
  ...
  if(encoded){project=await importSnapshot(decodeState(encoded),encoded);toast('??????????')}
  ...
  if(!project)project=await api('/api/projects',{method:'POST',body:JSON.stringify({name:'???? 01'})});
...
function renderSites(){
  ...
  $('#siteCount').textContent=`?? ${state.project.listings.filter((item)=>item.selected).length} ???`;
...
function renderResults(){
  ...
  if(!selected.length){$('#resultRows').innerHTML='<tr><td class="empty-row" colspan="8">???????????</td></tr>';return}
  $('#resultRows').innerHTML=selected.map((listing)=>{
    ...
    return `<tr><td class="country-cell">${country.flag} ${marketCode(country.code)}<small>${escapeHtml(country.name)}</small></td><td><label class="price-input"><span>${escapeHtml(listing.symbol)}</span><input type="number" min="0" step="0.01" value="${listing.sale_price||''}" placeholder="0.00" data-price="${listing.country_code}"></label></td><td>${number(commission)}%<span class="subvalue">${listing.referral_rate_override==null?escapeHtml(listing.matched_category||'????'):'????'}</span></td><td>${result?`${escapeHtml(result.symbol)}${number(result.fba_fee)}`:'?'}<span class="subvalue">${escapeHtml(result?.size_tier_name||'???')}</span></td><td>${result?`${escapeHtml(result.symbol)}${number(result.freight_fee)}`:'?'}</td><td class="${cls}">${priced&&result?`${result.profit<0?'-':''}${escapeHtml(result.symbol)}${number(Math.abs(result.profit))}`:'?'}</td><td class="profit-rate-cell ${cls}"><b>${priced&&result?`${number(result.profit_rate,1)}%`:'?'}</b>${priced?profitInfoIcon(result):''}</td><td><div class="row-actions"><button class="row-copy-button" type="button" data-copy-listing="${listing.country_code}">??</button><a class="row-card-link" href="./site-card.html?project=${state.project.id}&country=${listing.country_code}" target="_blank" rel="noopener">????</a></div></td></tr>`;
  }).join('');
...
function yesNoLabel(value){return value==null?'?':Number(value)?'?':'?'}
function competitorImage(item){return item.image_url?`<img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.name||item.asin||'??')}" loading="lazy" referrerpolicy="no-referrer">`:'<span>??</span>'}
function competitorRow(item,country){
  const rateClass=item.profit_rate==null?'':Number(item.profit_rate)>=0?'positive':'negative';
  return `<tr title="${escapeHtml(item.name||item.asin||'??')}">
    <td class="competitor-image">${competitorImage(item)}</td>
    <td><label class="competitor-price"><span>${escapeHtml(country.symbol)}</span><input type="number" min="0" step="0.01" data-competitor-price="${item.id}" value="${item.sale_price||''}" placeholder="0.00"></label><span class="competitor-name-hint">${escapeHtml(item.name||item.asin||'?????')}</span></td>
    <td class="competitor-link">${item.product_url?`<a href="${escapeHtml(item.product_url)}" target="_blank" rel="noopener">${escapeHtml(item.asin||'????')}</a>`:'?'}</td>
    ...
    <td class="profit-rate-cell competitor-profit ${rateClass}"><b>${item.profit_rate==null?'?':`${number(item.profit_rate,1)}%`}</b>${profitInfoIcon(item.calculation)}<button class="cost-button" type="button" data-competitor-cost="${item.id}">${item.uses_project_defaults?'????':'????'} ? ?${number(item.cost_cny)}</button></td>
    <td><div class="competitor-actions"><button class="delete-competitor" type="button" data-delete-competitor="${item.id}">??</button></div></td>
  </tr>`;
}
function renderCompetitors(){
  ...
  if(!selected.length){$('#competitorGroups').innerHTML='<div class="competitor-stats-empty">????????</div>';renderCompetitorStats();return}
  $('#competitorGroups').innerHTML=selected.map((country)=>{
    ...
    const body=visible.length?visible.map((item)=>competitorRow(item,country)).join(''):'<tr><td class="competitor-empty" colspan="7">????????????? Excel</td></tr>';
    const sourceNames=[...new Set(rows.map((item)=>item.source_format).filter(Boolean).map((value)=>value==='seller_sprite'?'????':value==='helium10'?'H10':value))];
    return `<div class="competitor-site"><div class="competitor-site-head"><div><b>${country.flag} ${marketCode(country.code)} ${escapeHtml(country.name)}</b><small>${total} ???</small>${total>5?'<small class="display-limit">???????? 5 ?</small>':''}</div><div class="competitor-site-head-actions"><button class="copy-competitors" type="button" data-copy-competitors="${country.code}" ${visible.length?'':'disabled'}>????</button><button class="import-competitors" type="button" data-import-competitors="${country.code}">?? Excel</button><button class="add-competitor" type="button" data-add-competitor="${country.code}">+ ????</button><button class="clear-competitors" type="button" data-clear-competitors="${country.code}" ${total?'':'disabled'}>????</button></div></div><div class="competitor-table-wrap"><table class="competitor-table"><thead><tr><th>??</th><th>??</th><th>????</th><th>??????????</th><th>??</th><th>?????</th><th>??</th></tr></thead><tbody>${body}</tbody></table></div><div class="competitor-import-note"><b>?????</b>${sourceNames.length?sourceNames.map(escapeHtml).join('?'):'????'}????????????????? ASIN ????? Excel ???? 30 ????????????????????? Excel ?????????????</div></div>`;
  }).join('');
...
function renderCompetitorStats(){
  ...
  for(const country of state.bootstrap.countries){
    ...
    cards.push(`<div class="competitor-stat"><b>${country.flag} ${marketCode(country.code)} ${escapeHtml(country.name)}</b><div class="competitor-stat-metrics"><span><small>??????</small>${number(averageSales,0)}</span><span><small>???????</small>${escapeHtml(country.symbol)}${number(averageRevenue,2)}</span><span><small>???????</small>${number(averageProfit,1)}%</span></div><small>?? ${divisor} ??????? ? ? ${Number(state.competitorCounts[country.code]??filled.length)} ???</small></div>`);
  }
  $('#competitorStats').innerHTML=cards.join('')||'<div class="competitor-stats-empty">?????????????????????</div>';
}
...
async function importCompetitorExcel(event){
  ...
  try{
    ...
    if(!parsed.rows.length)throw new Error('Excel ???????????');
    ...
    await loadCompetitors();saving(false);toast(`???? ${result.imported} ???? ${result.created}????? ${result.updated}${result.discarded?`????? ${result.discarded} ?`:''}`);
...
async function copyCompetitorTable(code){
  ...
  await writeRows(data);toast(`??? ${marketCode(code)} ?? ${rows.length} ???????????`);
}
async function copyCompetitorStats(){
  ...
  await writeRows(data);toast(`??? ${data.length} ????????????????`);
}
...
async function deleteCompetitor(id){
  saving(true);try{await api(`/api/competitors/${id}`,{method:'DELETE'});await loadCompetitors();saving(false);toast('???????')}catch(error){saving(false,true);toast(error.message)}
}
async function clearCompetitors(code){
  ...
  if(!total||!confirm(`???? ${marketCode(code)} ${country.name} ??? ${total} ???????`))return;
  saving(true);try{const result=await api(`/api/projects/${state.project.id}/competitors?country_code=${encodeURIComponent(code)}`,{method:'DELETE'});await loadCompetitors();saving(false);toast(`??? ${result.deleted} ?????`)}catch(error){saving(false,true);toast(error.message)}
}
function openCostModal(id){
  ...
  const country=state.bootstrap.countries.find((row)=>row.code===item.country_code);$('#competitorCostSubtitle').textContent=`${country.flag} ${marketCode(country.code)} ? ${item.name||'?????'}`;$('#competitorDefaultsState').textContent=item.uses_project_defaults?'??????????????????????':'??????????????????';$('#competitorCostModal').hidden=false;
}
...
async function confirmProjectDelete(){
  ...
  saving(true);try{await api(`/api/projects/${project.id}`,{method:'DELETE'});state.bootstrap=await api('/api/bootstrap');state.project=state.bootstrap.projects.length?await api(`/api/projects/${state.bootstrap.projects[0].id}`):await api('/api/projects',{method:'POST',body:JSON.stringify({name:'???? 01'})});state.shareKey=localStorage.getItem(`margingo-embed-key:${state.project.id}`)||newShareKey();localStorage.setItem(`margingo-embed-key:${state.project.id}`,state.shareKey);history.replaceState(null,'',`?project=${state.project.id}`);await refreshProjects();fillProduct();await calculate();await loadCompetitors();saving(false);toast('?????')}
  catch(error){saving(false,true);toast(error.message)}
}

function applyParsedDimensions(parsed){
  const fields=$('#productFields');for(const key of ['length','width','height'])$(`[name="${key}"]`,fields).value=parsed[key];$('[name="dimension_unit"]',fields).value=parsed.unit;toast(`????${parsed.length} ? ${parsed.width} ? ${parsed.height} ${parsed.unit}`);state.pending=saveProduct();
}
function recognizeDimensions(text){
  const parsed=window.DimensionParser?.parseDimensions(text,formValue('dimension_unit')||'cm');if(!parsed){toast('????????????? 27.2 ? 12.5 ? 54 cm ???');return false}applyParsedDimensions(parsed);return true;
}
async function readDimensionsFromClipboard(){
  if(!navigator.clipboard?.readText)return toast('????????????????????????');
  try{const text=await navigator.clipboard.readText();if(!text.trim())return toast('??????????????');recognizeDimensions(text)}catch{toast('????????????????????')}
}
...
function toggleCompetitorPanel(){
  state.competitorExpanded=!state.competitorExpanded;const panel=$('.competitor-panel');panel.classList.toggle('collapsed',!state.competitorExpanded);$('#competitorToggle').setAttribute('aria-expanded',String(state.competitorExpanded));$('.competitor-toggle-label').firstChild.textContent=state.competitorExpanded?'?? ':'?? ';
}
function toggleMarketPanel(){
  state.marketExpanded=!state.marketExpanded;const panel=$('.market-panel');panel.classList.toggle('collapsed',!state.marketExpanded);$('#marketToggle').setAttribute('aria-expanded',String(state.marketExpanded));$('.market-toggle-label').firstChild.textContent=state.marketExpanded?'?? ':'?? ';
}

async function saveProduct(){
  const body={name:formValue('name').trim()||'?????',cost_cny:Number(formValue('cost_cny'))||0,weight:Number(formValue('weight'))||0,weight_unit:formValue('weight_unit'),length:Number(formValue('length'))||0,width:Number(formValue('width'))||0,height:Number(formValue('height'))||0,dimension_unit:formValue('dimension_unit')};
  saving(true);
...
async function saveCommission(){
  ...
  if(value!==null && (!Number.isFinite(value) || value<0 || value>100))return toast('??????? 0?100');
  saving(true);
  try{
    ...
    await calculate();await loadCompetitors();saving(false);toast(value===null?'???????????':`????????? ${number(value,2)}%`);
  }catch(error){saving(false,true);toast(error.message)}
}
async function toggleSite(code){
  const listing=state.project.listings.find((item)=>item.country_code===code);
  if(listing.selected&&state.project.listings.filter((item)=>item.selected).length===1)return toast('??????????');
  saving(true);
...
async function writeRows(rows,linkIndex=-1){
  ...
  const html=`<table><tbody>${rows.map((row)=>`<tr>${row.map((item,index)=>index===linkIndex?`<td><a href="${escapeHtml(item)}">??</a></td>`:`<td>${escapeHtml(item)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
...
async function copyListingResult(code){
  ...
  await writeRows([[`${marketCode(country.code)} ${country.name}`,`${listing.symbol}${number(price)}`,result?`${number(result.profit_rate,1)}%`:'?']]);toast(`??? ${marketCode(code)} ????`);flushDrafts().catch((error)=>toast(error.message));
}
async function copySiteProfitTable(){
  ...
  await writeRows(rows);toast(`??? ${rows.length} ??????`);flushDrafts().catch((error)=>toast(error.message));
}
...
function bindEvents(){
  ...
  $('#newProjectBtn').onclick=async()=>{
    state.project=await api('/api/projects',{method:'POST',body:JSON.stringify({name:`???? ${state.bootstrap.projects.length+1}`})});state.shareKey=newShareKey();localStorage.setItem(`margingo-embed-key:${state.project.id}`,state.shareKey);history.replaceState(null,'',`?project=${state.project.id}`);await refreshProjects();fillProduct();await calculate();await loadCompetitors();toast('?????');
  };
...
initialize().catch((error)=>{console.error(error);toast(`?????${error.message}`);$('#resultRows').innerHTML=`<tr><td class="empty-row" colspan="8">${escapeHtml(error.message)}</td></tr>`});
